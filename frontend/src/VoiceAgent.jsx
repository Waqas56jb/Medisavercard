/**
 * MediSaver Voice Agent — Galaxy Widget
...
export function VoiceAgent({ onClose }) {
  ...
  /* keep statusRef in sync for use inside rAF loop */
      ...
      try {
        ...
        pc.ontrack = async (e) => {
          if (audioEl.srcObject !== e.streams[0]) {
            ...
            try { await audioEl.play(); } catch (pe) { console.warn('[VoiceAgent] play:', pe.message); }
          }
        };
  ...
  useEffect(() => {
    ...
    particlesRef.current = Array.from({ length: 420 }, () => {
      ...
      const radius = Math.pow(Math.random(), 0.5) * R * 0.85;
      const speed  = (0.0004 + Math.random() * 0.0008) * (Math.random() > 0.5 ? 1 : -1);
      return {
        angle, baseRadius: radius, radius, speed,
        hue: Math.random() * 80 + 260,
        layer: Math.floor(Math.random() * 3),
        size: Math.random() * 2 + 0.5, alpha: Math.random() * 0.7 + 0.3,
        wavePhase: Math.random() * Math.PI * 2, waveAmp: Math.random() * 12 + 3
      };
    });
    ...
    function drawStars(t) {
      ...
      stars.forEach(s => {
        ...
        sCtx.fillStyle = `rgba(255,255,255,${s.a.toFixed(2)})`; sCtx.fill();
      });
    }

    function drawGalaxy(energy, time) {
      gCtx.clearRect(0, 0, 220, 220);
      const hs = (time * 0.3) % 360;

      /* nebula bg */
      const bg = gCtx.createRadialGradient(CX, CY, 0, CX, CY, R);
      bg.addColorStop(0,   `hsla(${260+hs*.1},80%,8%,.95)`);
      bg.addColorStop(0.4, `hsla(${280+hs*.15},70%,5%,.9)`);
      bg.addColorStop(1,   `hsla(${300+hs*.08},60%,2%,.85)`);
      gCtx.save(); gCtx.beginPath(); gCtx.arc(CX,CY,R,0,Math.PI*2); gCtx.fillStyle=bg; gCtx.fill(); gCtx.restore();

      /* nebula clouds */
      for (let i = 0; i < 3; i++) {
        const a = time * 0.0005 + i * 2.1;
        const nx = CX + Math.cos(a) * 35, ny = CY + Math.sin(a) * 25;
        const ng = gCtx.createRadialGradient(nx,ny,0,nx,ny,55);
        const nh = (170 + i*80 + hs*.5) % 360;
        ng.addColorStop(0, `hsla(${nh},90%,55%,${0.06+energy*0.08})`);
        ng.addColorStop(1, `hsla(${nh},80%,40%,0)`);
        gCtx.save(); gCtx.beginPath(); gCtx.arc(CX,CY,R,0,Math.PI*2); gCtx.clip();
        gCtx.fillStyle=ng; gCtx.fillRect(0,0,220,220); gCtx.restore();
      }

      /* particles */
      particlesRef.current.forEach(p => {
        p.angle  += p.speed * (1 + energy * 2.5);
        p.radius  = p.baseRadius + p.waveAmp * Math.sin(time * 0.002 + p.wavePhase) * energy;
        const x = CX + Math.cos(p.angle) * p.radius;
        const y = CY + Math.sin(p.angle) * p.radius * 0.55;
        const dist = Math.sqrt((x-CX)**2+(y-CY)**2);
        if (dist > R) return;
        const ph = (p.hue + hs*.8 + time*.05) % 360;
        const al = p.alpha * (1 - dist/R*0.4) * (0.6 + energy*0.4);
        const sz = p.size * (1 + energy*1.2);
        gCtx.save();
        gCtx.globalCompositeOperation = p.layer===1 ? 'lighter' : 'screen';
        gCtx.beginPath(); gCtx.arc(x,y,sz,0,Math.PI*2);
        gCtx.fillStyle = `hsla(${ph},90%,70%,${al.toFixed(2)})`; gCtx.fill();
        if (p.size > 1.5) {
          const glow = gCtx.createRadialGradient(x,y,0,x,y,sz*3.5);
          glow.addColorStop(0, `hsla(${ph},100%,80%,${(al*0.4).toFixed(2)})`);
          glow.addColorStop(1, `hsla(${ph},90%,60%,0)`);
          gCtx.fillStyle=glow; gCtx.beginPath(); gCtx.arc(x,y,sz*3.5,0,Math.PI*2); gCtx.fill();
        }
        gCtx.restore();
      });
      ...
      /* core glow */
      const ch = (180+hs)%360;
      const core = gCtx.createRadialGradient(CX,CY,0,CX,CY,30+energy*20);
      core.addColorStop(0,   `hsla(${ch},100%,90%,${0.3+energy*0.5})`);
      core.addColorStop(0.5, `hsla(${(ch+40)%360},90%,70%,${0.1+energy*0.2})`);
      core.addColorStop(1,   `hsla(${(ch+80)%360},80%,50%,0)`);
      gCtx.save(); gCtx.globalCompositeOperation='screen'; gCtx.fillStyle=core;
      gCtx.beginPath(); gCtx.arc(CX,CY,30+energy*20,0,Math.PI*2); gCtx.fill(); gCtx.restore();
    }

    function updateBars(t, energy) {
      barsRef.current.forEach((b, i) => {
        if (!b) return;
        const base  = Math.sin(t*0.003 + i*0.35) * 0.5 + 0.5;
        const wave2 = Math.sin(t*0.005 + i*0.6)  * 0.3 + 0.3;
        const st = statusRef.current;
        const active = st==='listening'||st==='speaking'||st==='ready';
        let h = active ? 4 + (base*wave2 + Math.random()*0.3)*44 : 4 + base*10;
        b.style.height  = Math.round(h) + 'px';
        b.style.opacity = String(0.4 + (h/48)*0.6);
      });
    }

    function loop(t) {
      const st = statusRef.current;
      const isAct = st==='listening'||st==='ready'||st==='speaking';
      const tgt = isAct ? 0.55 + Math.sin(t*0.004)*0.45 : 0.1;
      energyRef.current += (tgt - energyRef.current) * 0.06;
      drawStars(t);
      drawGalaxy(energyRef.current, t);
      updateBars(t, energyRef.current);
  ...
  if (status === 'connecting') return createPortal(
    <div className="va-gl-overlay" aria-busy="true">
      <div className="va-gl-backdrop" />
      <div className="va-loader-dots" aria-hidden><span /><span /><span /></div>
    </div>, document.body);

  /* ── PHASE 2: error ── */
  if ((status==='error'||status==='denied') && errInfo) return createPortal(
    <div className="va-gl-overlay" role="alertdialog" aria-modal="true">
      <div className="va-gl-backdrop" />
      <div className="va-err-card">
        <span className="va-err-icon">{errInfo.icon}</span>
        <p className="va-err-text">{errInfo.text}</p>
        <button type="button" className="va-end-btn" onClick={stop}>Close</button>
      </div>
    </div>, document.body);

  /* ── PHASE 3: Galaxy widget ── */
  return createPortal(
    <div className="va-gl-overlay" role="dialog" aria-modal="true" aria-label="Voice agent active">
      <div className="va-gl-backdrop" />
      ...
      <div className="va-gl-widget">
        ...
        <div className={`va-gl-orb${isOrbActive ? ' va-active' : ''}`} onClick={stop} title="Click to end session">
          ...
          <div className="va-pulse-ring" aria-hidden />
          <div className="va-pulse-ring" aria-hidden />
          <div className="va-pulse-ring" aria-hidden />
          <div className="va-glass"  aria-hidden />
          <div className="va-ring"   aria-hidden />
          <div className="va-ring2"  aria-hidden />
          <div className="va-mic-center" aria-hidden>
            <div className="va-mic-icon">
        ...
        <div className="va-wave-bar" aria-hidden>
          {Array.from({ length: 48 }, (_, i) => {
            const hue = 170 + (i / 48) * 100;
            return (
              <div
                key={i}
                className="va-bar"
                style={{ height: '4px', background: `linear-gradient(to top,hsl(${hue+80},90%,60%),hsl(${hue},100%,70%))` }}
        ...
        <button type="button" className="va-end-btn" onClick={stop} aria-label="End voice session">
          ■ End Session
        </button>
      </div>
    </div>, document.body);
